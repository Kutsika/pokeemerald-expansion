    ...
    case BATTLE_INTRO_STATE_DRAW_SPRITES:
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            else
            {
                ...
                if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
                {
                    const struct Trainer *trainerData = NULL;

                    if ((gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS) && GetBattlerTrainer(battler) == B_TRAINER_OPPONENT_B)
                        trainerData = GetTrainerStructFromId(TRAINER_BATTLE_PARAM.opponentB);
                    else
                        trainerData = GetTrainerStructFromId(TRAINER_BATTLE_PARAM.opponentA);

                    if (trainerData && !IsTrainerRandomized(trainerData))
                        gBattleMons[battler].ability = GetSpeciesAbility(gBattleMons[battler].species, gBattleMons[battler].abilityNum);
                    else
